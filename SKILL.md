...
description: 黑客松/比赛参赛**全流程一条龙**:题目解析 → idea → 计划+PRD → 路演 PPT → 项目说明书(飞书文档+架构图)。用户提"路演 PPT / PRD / 产品规划 / 项目说明书 / 参赛文档 / 黑客松 / 比赛"任一,且要**多份文档**时触发本 skill(产出全套)。已有项目+题目可跳过 idea、直达计划+PRD+PPT。PPT 用 skill 内置 svg2pptx(SVG→可编辑 PPTX,借鉴 ppt-master),视觉风格不锁定(NeoBrutalism/极简学术/商务蓝等预设或自定义)。IO skill,不写代码、不开 PR。
