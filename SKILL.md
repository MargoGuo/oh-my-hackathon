...
每次启动前,检查 skill 目录有无上游更新,有更新询问用户是否拉取:

```bash
git -C "<SKILL_ROOT>" fetch --quiet
git -C "<SKILL_ROOT>" rev-list --count HEAD..@{u}  # >0 = 有更新
```

有更新 → 询问是否 `git pull --ff-only`;用户拒绝则用当前版本;网络不通 / 无 upstream 则跳过,不阻塞流程。
...
<HARD-GATE>启动前必澄清(条件不满足禁止进 Stage 1):
- **题目 / brief**(必需):主题 / 约束 / **评审标准(rubric)** / 交付物要求 —— 拿不到让用户粘贴,不凭空编
- 可选(有则更准):已有项目 repo / 团队技术偏好 / 视觉风格 / 飞书文件夹 token

缺题目 → 停下来问用户,不进 idea / 计划 / PPT。这是硬门控,不是建议。
